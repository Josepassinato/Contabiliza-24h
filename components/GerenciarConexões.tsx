...
const GerenciarConexoes: React.FC<GerenciarConexoesProps> = ({ onBack }) => {
    ...
    const handleSaveConnection = async (platformId: string, credentials: { apiKey: string, apiSecret: string }) => {
        try {
            ...
            addNotification(`Plataforma conectada com sucesso!`, 'success');
        } catch(error) {
            addNotification(`Erro ao conectar plataforma.`, 'error');
            console.error(error);
        }
    };
    ...
    const handleDisconnect = async (platformId: string) => {
        try {
            ...
            addNotification(`Plataforma desconectada.`, 'success');
        } catch(error) {
            addNotification(`Erro ao desconectar plataforma.`, 'error');
            console.error(error);
        }
    };
